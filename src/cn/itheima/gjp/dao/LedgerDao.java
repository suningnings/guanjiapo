...
public class LedgerDao {
    ...
    public Double queryTotalMonetByParent(String parent) {
        ...
        try {
            ...
            String p2 = year+"%";
            Object[] params = { parent, p2 };
            System.out.println((Double) qr.query(sql, new ScalarHandler(),params));
            Double totalMoney = (Double) qr.query(sql, new ScalarHandler(),	params);
    ...
    public List<Object[]> quarySumMoneyBySort(String parent) {
        // TODO Auto-generated method stub
        try {
    ...
    public List<Ledger> queryAllLedger() throws SQLException {
        ...
        // 我要的数据
        /*
         * 需要显示的数据 与 对应的表 账务表 gjp_ledger 显示数据 ID 收/支 分类名 金额 账户 创建时间 说明
         *
         * 数据库字段 lid parent sid money account createtime ldesc 账务id 父分类 分类id 金额
         * 账户 创建时间 描述
         *
         * 我们 要的 ID 收/支 金额 账户 创建时间 说明 从gjp_ledger表中获取 我们 要的分类名 从 gjp_sort表中获取
         *
         * 需要两张表 单独获取 可以 low 使用多表查询 能否完成呢? 答案是 可以 怎么做 来到数据库 操作这里 拼接一下
         */
         ...
         */
        List<Ledger> list = qr.query(sql, new BeanListHandler<Ledger>(
                Ledger.class), params);
    ...
    /**
     * 根据时间查询 数据库 这里面需要注意的是 除了时间以外的 其他条件 是 可以有可以没有的
     *
     * 这里面 要执行sql 是需要拼接完成的
     *
     * 四个参数 beginTime endTime parent sname 返回值 List<Ledger>
     *
     * 小概念 集合转换成数组 toArray
     *
     * @throws SQLException
     */
    // public List<Ledger> queryLedger(String beginTime,String endTime,String
    // parent,String sname) throws SQLException{
    //
    //
    // // 1:初始化 sql语句
    // // 因为在当前 时间 条件是必须写出来的
    ...
    // StringBuilder("select * from gjp_ledger where createTime between ? and ?");
    //
    // //定义一个集合 用来接收参数列表
    ...
    //
    // List<Ledger> list = qr.query(sql, new BeanListHandler<>(Ledger.class),
    // params);
    //
    // return list;
    // }
    //
    /**
     * 根据时间查询 数据库 第二种方式 多表方式 这里面需要注意的是 除了时间以外的 其他条件 是 可以有可以没有的
     *
     * 这里面 要执行sql 是需要拼接完成的
     *
     * 四个参数 beginTime endTime parent sname 返回值 List<Ledger>
    ...
    public List<Ledger> queryLedger(String beginTime, String endTime,
        ...
        StringBuilder sb = new StringBuilder(
                "select l.lid,l.parent,s.sname,l.money,l.account,l.createtime,l.ldesc FROM gjp_sort AS s ,gjp_ledger AS l WHERE s.sid=l.sid and l.createTime between ? and ?");
    ...
    /**
     * 根据Ledger对象 存储 到数据库表中 参数Ledger对象
     *
     * @throws SQLException
     */
    public void addLedger(Ledger ledger) throws SQLException {
        ...
        Object[] params = { ledger.getParent(), ledger.getMoney(),
                ledger.getSid(), ledger.getAccount(), ledger.getCreatetime(),
                ledger.getLdesc() };
